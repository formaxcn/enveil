...
exports.DualLayerHighlightingValidator = void 0;
const CloudHighlighter_1 = require("../components/CloudHighlighter");
/**
...
class DualLayerHighlightingValidator {
    constructor() {
        this.testResults = [];
        this.highlighter = new CloudHighlighter_1.CloudHighlighter();
    }
    /**
     ...
     */
    async testBasicDualLayerCoordination() {
        console.log('\n--- Test 1: Basic Dual-Layer Coordination ---');
        try {
     ...
     */
    async testVisualDistinctionEnforcement() {
        console.log('\n--- Test 2: Visual Distinction Enforcement ---');
        try {
            // Clear any existing highlighting
            this.highlighter.removeHighlighting();
            ...
            const hasDistinctColors = roleColors.every(roleColor => this.calculateColorDistance(accountColor, roleColor) > 50 // Minimum distance threshold
            );
     ...
     * Validates Requirement 8.3
     */
    async testColorSeparationValidation() {
        console.log('\n--- Test 3: Color Separation Validation ---');
        try {
            ...
            for (const combination of colorCombinations) {
                ...
                const roles = combination.roles.map((color, index) => this.createTestRole(`role-${index}`, [`keyword${index}`], '#ffffff', color));
                // Check color separation
                const accountDistance = combination.roles.every(roleColor => this.calculateColorDistance(combination.account, roleColor) > 30);
                if (!accountDistance) {
     ...
     */
    async testHighlightingHierarchy() {
        console.log('\n--- Test 4: Highlighting Hierarchy Maintenance ---');
        try {
            // Clear any existing highlighting
            this.highlighter.removeHighlighting();
     ...
     */
    async testSimultaneousLayerFunctionality() {
        console.log('\n--- Test 5: Simultaneous Layer Functionality ---');
        try {
            // Clear any existing highlighting
            this.highlighter.removeHighlighting();
     ...
     */
    createTestAccount(color = '#ff6b6b') {
        return {
     ...
     */
    createTestRole(id, keywords, textColor, backgroundColor) {
        return {
            id: id,
            name: `Test ${id}`,
            enable: true,
...
exports.DualLayerHighlightingValidator = DualLayerHighlightingValidator;
