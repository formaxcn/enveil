 ...
 * Specifically designed for Alibaba Cloud SSO account selection pages
 */
export class AliyunAccountSelectionHandler {
    private static readonly ACCOUNT_HIGHLIGHT_CLASS = 'enveil-aliyun-account-highlight';
    private static readonly ROLE_HIGHLIGHT_CLASS = 'enveil-aliyun-role-highlight';
    private static readonly STYLE_ID = 'enveil-aliyun-account-selection-styles';
    ...
    /**
     * Applies highlighting to Aliyun account selection page.
     */
    ...
    public removeHighlighting(): void {
        this.highlightedAccounts.forEach((elements) => {
            elements.forEach(el => {
                ...
                el.classList.remove(AliyunAccountSelectionHandler.ACCOUNT_HIGHLIGHT_CLASS);
            });
        });
    ...
    /**
     * Highlights a specific Aliyun account container.
     */
    ...
    /**
     * Finds Aliyun account containers that match the given account.
     */
    ...
    /**
     * Checks if an Aliyun account container matches the given account.
     */
    ...
    /**
     * Applies background color to an Aliyun account container.
     */
    private applyAccountBackground(element: HTMLElement, account: CloudAccount): void {
        ...
        element.classList.add(AliyunAccountSelectionHandler.ACCOUNT_HIGHLIGHT_CLASS);
    }

    /**
     * Highlights role keywords within an Aliyun account container.
     */
    ...
    private highlightRoleKeywordsInElement(element: HTMLElement, roles: CloudRole[], highlightColor?: string): void {
        for (const role of roles) {
            ...
            const walker = document.createTreeWalker(
                ...
                NodeFilter.SHOW_TEXT,
                {
                    acceptNode: (node) => {
                        ...
                        if (parent.classList.contains(AliyunAccountSelectionHandler.ROLE_HIGHLIGHT_CLASS)) {
                            return NodeFilter.FILTER_REJECT;
                        }
            ...
            const className = `${AliyunAccountSelectionHandler.ROLE_HIGHLIGHT_CLASS}-${role.id}`;
            ...
            textNodes.forEach(textNode => {
                ...
                const modifiedText = originalText.replace(
                    regex,
                    `<span class="${className} ${AliyunAccountSelectionHandler.ROLE_HIGHLIGHT_CLASS}" data-role-id="${role.id}" data-original-text="$1">$1</span>`
                );
                ...
                if (modifiedText !== originalText && textNode.parentNode) {
                    ...
                    const highlightedElements = wrapper.querySelectorAll(`.${AliyunAccountSelectionHandler.ROLE_HIGHLIGHT_CLASS}`);
    ...
    private createStyles(): void {
        ...
        this.styleElement.id = AliyunAccountSelectionHandler.STYLE_ID;

        this.styleElement.textContent = `
            .${AliyunAccountSelectionHandler.ACCOUNT_HIGHLIGHT_CLASS} {
                transition: all 0.3s ease !important;
            }
            .${AliyunAccountSelectionHandler.ROLE_HIGHLIGHT_CLASS} {
