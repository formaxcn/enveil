    ...
    return (
        <div className="space-y-6">
            <div className="form-group">
                <label className="block text-sm font-bold text-gray-700 mb-2">Environment Name</label>
                <input
                    type="text"
            ...
            <div
                className="relative overflow-hidden rounded-3xl p-8 border-2 border-gray-200 transition-all duration-300"
                style={{
            ...
            >
                ...
                {config.flagEnable && (
                    <div
                        className={clsx(
                            "absolute w-32 py-1 text-[10px] font-bold text-center text-white shadow-lg pointer-events-none",
                ...
                <div className="grid grid-cols-2 gap-4 relative z-10">
                    <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-white/20 shadow-sm space-y-4">
                        <Switch
                    ...
                    <div className={clsx(
                        "bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-white/20 shadow-sm transition-all",
                        config.flagEnable ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"
                    )}>
                        <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">Position</label>
                        <select
                            value={config.Position}
                            onChange={(e) => onChange({ Position: e.target.value })}
                            className="w-full bg-transparent border-none text-sm font-medium focus:ring-0 cursor-pointer"
                        >
                ...
                <div className="mt-6 bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-white/20 shadow-sm">
                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-3">Theme Color</label>
                    <div className="flex flex-wrap gap-2 items-center">
                        {defaultColors.map((color, idx) => (
                            <button
                                ...
                                className={clsx(
                                    ...
                                    config.color?.toLowerCase() === color.toLowerCase() ? "border-blue-500 scale-110 ring-2 ring-blue-100" : "border-transparent"
                                )}
                        ...
                        <button
                            ...
                            className={clsx(
                                ...
                                !isDefaultColor ? "border-blue-500 scale-110" : "border-dashed border-gray-300 bg-gray-50 text-gray-400"
                            )}
